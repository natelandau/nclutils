...
@freeze_time("2024-03-15 12:34:56.789012")
def test_new_timestamped_uid_format(debug) -> None:
    """Verify new_timestamped_uid() generates correctly formatted string."""
...
@freeze_time("2024-03-15 12:34:56.789012")
def test_new_timestamped_uid_custom_bits() -> None:
    """Verify new_timestamped_uid() generates correct length with custom bits."""
    # Given: Different bit values to test
    test_bits = [32, 64, 128, 256]

    for bits in test_bits:
        # When: Generating a timestamped UID with specified bits
        uid = new_timestamped_uid(bits)
...
@freeze_time("2024-03-15 12:34:56.789012")
def test_new_timestamped_uid_uniqueness() -> None:
    """Verify new_timestamped_uid() generates unique strings."""
    # When: Generating multiple timestamped UIDs
    uids = {new_timestamped_uid() for _ in range(100)}
...
@freeze_time("2024-03-15 12:34:56.789012")
def test_new_timestamped_uid_components() -> None:
    """Verify new_timestamped_uid() contains all required components."""
    # When: Generating a timestamped UID
    uid = new_timestamped_uid()
...
def test_new_timestamped_uid_zero_microseconds(debug) -> None:
    """Verify new_timestamped_uid() handles zero microseconds correctly."""
