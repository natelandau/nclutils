---
name: nclutils
description: Use whenever writing or editing Python code in a project that depends on (or could depend on) the `nclutils` package. Covers correct import paths, the public API surface, and the gotchas that downstream code routinely gets wrong: the `nclutils.pp` vs stdlib `logging` split, `nclutils.sh.run_command` error handling and return values, when to reach for `nclutils.fs` / `nclutils.git` / `nclutils.strings` / `nclutils.utils` instead of rewriting equivalents with `shutil`, `subprocess`, ad-hoc regex, or `uuid`, and how the `pp` spinner and verbosity gates work. Read this BEFORE writing any code that touches the package, including throwaway scripts.
---
...
- The project's `pyproject.toml`, `requirements.txt`, or lockfile lists `nclutils`.
- You see `import nclutils` or `from nclutils...` anywhere in the file you are editing.
- You are about to write code that does something `nclutils` already does well (see [What lives where](#what-lives-where)). If the package is already a dependency, prefer the existing helper over a hand-rolled equivalent.

If `nclutils` is not installed and not in scope to add, this skill does not apply.
...
| Copy a directory recursively                        | `nclutils.fs.copy_directory(src, dst)`                                                                   | Python 3.12+ only (uses `Path.walk`).                                          |
...
> [!NOTE]
> Older guidance that mentions `loguru` is stale. The project migrated off `loguru` in commit 6cafada; both channels above are the current model.
...
`nclutils` supports Python 3.10+. A handful of features (notably `nclutils.fs.copy_directory`) gate themselves on Python 3.12 via `nclutils.utils.check_python_version`. If the downstream project's Python floor is 3.10 or 3.11 and you need one of those features, gate the same way rather than raising the project minimum.
